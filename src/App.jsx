...
function BookPage(props) {
  ...
  function confirm(){ var st=stus.find(function(s){ return s.email.toLowerCase()===form.email.toLowerCase(); }); if(!st){ st={id:uid(),name:form.name,email:form.email,phone:form.phone,enrolled:tod(),progress:Object.fromEntries(TOPICS.map(function(t){ return [t,false]; })),examReady:false,examDate:null,examResult:null}; setStus([...stus,st]); } setBks([...bks,...slots.map(function(s){ return {id:uid(),sid:st.id,sName:form.name,sEmail:form.email,sPhone:form.phone,date:s.date,hour:s.hour,dur:s.dur,status:"confirmed",notes:form.notes}; })]); setStep(4); }
