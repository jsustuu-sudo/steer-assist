      ...
      React.createElement("div",{className:"d-show d-hero",style:{display:"none"}},
        React.createElement("div",{className:"d-hero-text"},
          React.createElement("div",{style:{display:"flex",alignItems:"center",gap:14,marginBottom:24}},
            ...
            React.createElement("div",null,React.createElement("div",{style:{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:22,fontWeight:800,color:"#fff",lineHeight:1}},"Steer Assist"),React.createElement("div",{style:{fontSize:13,color:"rgba(255,255,255,0.65)",marginTop:4}},"Melbourne, Victoria"),React.createElement("div",{className:"sbadge",style:{display:"inline-block",marginTop:6,padding:"3px 12px",borderRadius:20,fontSize:10,fontWeight:800,color:NV}},"EX-VICROADS OFFICER"))
          ),
        ...
        React.createElement("div",{style:{display:"flex",gap:12}},
          tests.slice(0,3).map(function(t){
            return React.createElement("div",{key:t.id,style:{width:240,background:"#fff",borderRadius:16,padding:16,boxShadow:"0 4px 16px rgba(0,0,0,0.06)",flexShrink:0}},
              ...
              React.createElement("div",{style:{display:"flex",alignItems:"center",gap:8}},React.createElement("div",{style:{width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg,"+B+","+NV+")",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:12,flexShrink:0}},t.name[0]),React.createElement("div",{style:{fontSize:13,fontWeight:700,color:NV}},t.name))
            );
          })
        )
      )
      ...
      React.createElement("div",{className:"d-rev-grid"},
        tests.slice(0,3).map(function(t){
          return React.createElement("div",{key:t.id,style:{background:"#fff",borderRadius:20,padding:28,boxShadow:"0 4px 20px rgba(0,0,0,0.06)"}},
            ...
            React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10}},React.createElement("div",{style:{width:38,height:38,borderRadius:"50%",background:"linear-gradient(135deg,"+B+","+NV+")",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:14,flexShrink:0}},t.name[0]),React.createElement("div",{style:{fontSize:14,fontWeight:700,color:NV}},t.name))
          );
        })
      )
    ),
...
function BookPage(props) {
  ...
  async function confirm(){
    ...
    if(!st){
      ...
      await sbUpsert("students",{id:st.id,name:st.name,email:st.email,phone:st.phone,enrolled:st.enrolled,progress:st.progress,exam_ready:st.examReady,exam_date:st.examDate,exam_result:st.examResult});
      setStus([...stus,st]);
    }
    var newBks=slots.map(function(s){ return {id:uid(),sid:st.id,sName:form.name,sEmail:form.email,sPhone:form.phone,date:s.date,hour:s.hour,dur:s.dur,status:"confirmed",notes:form.notes}; });
    for(var bi=0;bi<newBks.length;bi++){ var nb=newBks[bi]; await sbUpsert("bookings",{id:nb.id,sid:nb.sid,s_name:nb.sName,s_email:nb.sEmail,s_phone:nb.sPhone,date:nb.date,hour:nb.hour,dur:nb.dur,status:nb.status,notes:nb.notes}); }
    setBks([...bks,...newBks]);
    try{
      for(var i=0;i<slots.length;i++){
        var s=slots[i];
        await fetch("https://api.emailjs.com/api/v1.0/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({service_id:EJS_SVC,template_id:"template_vqz1p1b",user_id:EJS_KEY,template_params:{student_name:form.name,student_email:form.email,lesson_date:fmtD(s.date),lesson_time:fmtT(s.hour),pickup_location:form.notes||"To be confirmed"}})});
      }
