          ...
          React.createElement("div",{style:{flex:1,marginBottom:12,minHeight:90}},
            p.feats.map(function(f){
              return React.createElement("div",{key:f,style:{display:"flex",alignItems:"flex-start",gap:5,marginBottom:4}},
                React.createElement("div",{style:{width:14,height:14,borderRadius:"50%",background:GR+"20",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}},React.createElement(Ico,{n:"check",sz:9,c:GR,sw:3})),
...
function BookPage(props) {
  ...
  async function confirm(){ var st=stus.find(function(s){ return s.email.toLowerCase()===form.email.toLowerCase(); }); if(!st){ st={id:uid(),name:form.name,email:form.email,phone:form.phone,enrolled:tod(),progress:Object.fromEntries(TOPICS.map(function(t){ return [t,false]; })),examReady:false,examDate:null,examResult:null}; setStus([...stus,st]); } var newBks=slots.map(function(s){ return {id:uid(),sid:st.id,sName:form.name,sEmail:form.email,sPhone:form.phone,date:s.date,hour:s.hour,dur:s.dur,status:"confirmed",notes:form.notes}; }); setBks([...bks,...newBks]); try{ for(var i=0;i<slots.length;i++){ var s=slots[i]; await fetch("https://api.emailjs.com/api/v1.0/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({service_id:EJS_SVC,template_id:"template_vqz1p1b",user_id:EJS_KEY,template_params:{student_name:form.name,student_email:form.email,lesson_date:fmtD(s.date),lesson_time:fmtT(s.hour),pickup_location:form.notes||"To be confirmed"}})}); } }catch(e){ console.log("Email error",e); } setStep(4); }
  ...
  return React.createElement("div",null,
    ...
    React.createElement("div",{style:{padding:"16px 16px 0"},className:"wrap"},
      React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:12,marginBottom:16}},
        [["Today",todayB.length,B,"cal"],["Upcoming",upcoming,SK,"award"],["Students",stus.length,GR,"user"],["Exams",exams.length,GD,"trophy"]].map(function(row){ return React.createElement("div",{key:row[0],style:{background:"#fff",borderRadius:16,padding:16,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}},React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}},React.createElement("div",{style:{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:28,fontWeight:800,color:row[2]}},row[1]),React.createElement("div",{style:{width:36,height:36,borderRadius:10,background:row[2]+"15",display:"flex",alignItems:"center",justifyContent:"center"}},React.createElement(Ico,{n:row[3],sz:18,c:row[2]}))),React.createElement("div",{style:{fontSize:12,color:MT,marginTop:4,fontWeight:600}},row[0])); })
      ),
...
function StusPage(props) {
  ...
  var filtered=stus.filter(function(s){ var ms=s.name.toLowerCase().includes(search.toLowerCase())||s.email.toLowerCase().includes(search.toLowerCase()); if(filter==="exam-ready") return ms&&s.examReady&&!s.examResult; if(filter==="passed") return ms&&s.examResult==="pass"; if(filter==="in-progress") return ms&&!s.examReady; return ms; });
  if(sel){
