...
const PACKAGES=[
  ...
  {id:"p2",name:"5-Lesson Pack",icon:"⭐",lessons:5,desc:"Great starter pack. Build core skills with structured sessions.",features:["5 × 1 hour lessons","Pick-up & drop-off","Progress tracking","Topic checklist"],popular:false},
  {id:"p3",name:"10-Lesson Pack",icon:"🏆",lessons:10,desc:"Best value for learners aiming for a first-time test pass.",features:["10 × 1 hour lessons","Pick-up & drop-off","Full progress tracking","Exam readiness assessment","Test prep session"],popular:true},
];
  ...
  return(
    <div>
      ...
      <div style={{background:`linear-gradient(160deg, ${C.navy} 0%, ${C.blue} 55%, ${C.brand} 100%)`,padding:"28px 20px 0",position:"relative",overflow:"hidden",minHeight:360}}>
        ...
        {/* Google rating strip at very top */}
      ...
      {/* Real road animation — between hero and stats */}
                  ...
                  return(
                    <td key={d} style={{padding:2,borderBottom:`1px solid #F8FAFC`,borderLeft:`1px solid #F8FAFC`,height:46,width:54}}>
                      <button className="slot-btn" onClick={()=>toggle(d,h)} disabled={unavail}
                        ...
                        {sel?<span style={{color:C.green,fontWeight:900,fontSize:16}}>✓</span>
                          :bkd?<span style={{fontSize:10,color:"#B45309"}}>●</span>
    ...
    <div>
      ...
      <div style={{padding:16}}>
        ...
        <div style={{background:"#FEF3C7",borderRadius:12,padding:12,marginBottom:14,fontSize:12,color:"#92400E",lineHeight:1.55}}>
          ⚙️ <strong>Setup required:</strong> To receive form messages, create a free account at <strong>emailjs.com</strong> and replace the SERVICE_ID, TEMPLATE_ID and PUBLIC_KEY at the top of this file. <a href="https://emailjs.com" style={{color:C.brand,fontWeight:700}}>Get started →</a>
        </div>
        ...
        <div style={{background:"#fff",borderRadius:20,padding:20,boxShadow:"0 4px 20px rgba(0,0,0,0.06)"}}>
          ...
          {status==="error"&&<div style={{background:"#FEE2E2",borderRadius:10,padding:10,marginBottom:12,fontSize:13,color:C.red}}>⚠️ Please fill in your name, email and message, then try again.</div>}
      ...
      <div>
        ...
        <div style={{padding:16}}>
          ...
          <div style={{background:"#fff",borderRadius:18,padding:18,boxShadow:"0 4px 16px rgba(0,0,0,0.06)",marginBottom:12}}>
            ...
            {s.examResult?(<div style={{background:s.examResult==="pass"?"#D1FAE5":"#FEE2E2",borderRadius:12,padding:12}}><div style={{fontWeight:800,color:s.examResult==="pass"?"#065F46":C.red,fontSize:16}}>{s.examResult==="pass"?"🎉 PASSED!":"❌ Did not pass"}</div></div>)
            :!s.examReady?(<button onClick={()=>setExamR(s.id,true)} style={{padding:"10px 18px",background:`linear-gradient(135deg,${C.gold},${C.amber})`,border:"none",borderRadius:10,fontWeight:700,fontSize:13,color:C.navy,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>✓ Mark Exam Ready</button>)
            :(<div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>setExamRes(s.id,"pass")} style={{padding:"8px 14px",background:C.green,border:"none",borderRadius:10,fontWeight:700,fontSize:13,color:"#fff",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>✓ Passed</button>
              <button onClick={()=>setExamRes(s.id,"fail")} style={{padding:"8px 14px",background:C.red,border:"none",borderRadius:10,fontWeight:700,fontSize:13,color:"#fff",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>✗ Failed</button>
          ...
          <div style={{background:"#fff",borderRadius:18,padding:18,boxShadow:"0 4px 16px rgba(0,0,0,0.06)",marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              ...
              <button onClick={()=>setEditP(!editP)} style={{padding:"6px 12px",background:editP?"#D1FAE5":"#F4F7FB",border:"none",borderRadius:8,fontWeight:700,fontSize:12,color:editP?C.green:C.brand,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{editP?"Done ✓":"Edit"}</button>
            </div>
            ...
            {TOPICS.map(t=>(
              <div key={t} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid #F8FAFC`}}>
                <span style={{fontSize:13,color:s.progress[t]?"#065F46":C.slate,fontWeight:s.progress[t]?700:400}}>{s.progress[t]?"✓ ":"○ "}{t}</span>
                {editP&&<button onClick={()=>upProg(s.id,t,!s.progress[t])} style={{background:s.progress[t]?"#D1FAE5":"#EFF6FF",border:"none",borderRadius:8,padding:"4px 10px",cursor:"pointer",fontSize:12,fontWeight:700,color:s.progress[t]?C.green:C.brand,fontFamily:"'DM Sans',sans-serif"}}>{s.progress[t]?"✓":"Mark"}</button>}
              </div>
            ))}
          </div>
    ...
    <div>
      ...
      <div style={{padding:16}}>
        {showForm&&<div style={{background:"#fff",borderRadius:20,padding:20,boxShadow:"0 4px 20px rgba(0,0,0,0.06)",marginBottom:14}}>
          ...
          <button onClick={add} style={{width:"100%",padding:"12px",background:`linear-gradient(135deg,${C.green},#059669)`,border:"none",borderRadius:12,fontWeight:800,fontSize:14,color:"#fff",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>✓ Save</button>
        </div>}
